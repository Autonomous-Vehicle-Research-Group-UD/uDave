...
class RealSense435i_extended(object):
    """
...
cap2 = cv.VideoCapture('nvarguscamerasrc sensor_id=1 ! video/x-raw(memory:NVMM), width=1280, height=720, format=(string)NV12, framerate=(fraction)20/1 ! nvvidconv ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink' , cv.CAP_GSTREAMER)
    ...
    def __init__(self, width = WIDTH, height = HEIGHT, channels = CHANNELS, enable_rgb=True, enable_depth=True, enable_imu=True, device_id = None, driving_mirror_w_1 = 160,driving_mirror_h_1 = 120, driving_mirror_w_2 = 160,driving_mirror_h_2 = 120):
        self.device_id = device_id  # "923322071108" # serial number of device to use or None to use default
        self.enable_imu = enable_imu
        self.enable_rgb = enable_rgb
        self.enable_depth = enable_depth
        self.cam_a = cv2.VideoCapture('nvarguscamerasrc sensor_id=0 ! video/x-raw(memory:NVMM), width='+str(driving_mirror_w_1)+', height='+str(driving_mirror_h_1)+', format=(string)NV12, framerate=(fraction)20/1 ! nvvidconv ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink' , cv2.CAP_GSTREAMER)
        self.cam_b = cv2.VideoCapture('nvarguscamerasrc sensor_id=1 ! video/x-raw(memory:NVMM), width='+str(driving_mirror_w_2)+', height='+str(driving_mirror_h_2)+', format=(string)NV12, framerate=(fraction)20/1 ! nvvidconv ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink' , cv2.CAP_GSTREAMER)
        self.path = None
        self.width = width
        self.idx = 0
        self.height = height
        self.channels = channels
        self.resize = True#(width != WIDTH) or (height != height) or (channels != CHANNELS)
        if self.resize:
    ...
    def run_threaded(self):
        ret, frame = self.cam_a.read()
        ret, frame2 = self.cam_b.read()
        return self.color_image, self.depth_image, frame, frame2, self.acceleration_x, self.acceleration_y, self.acceleration_z, self.gyroscope_x, self.gyroscope_y, self.gyroscope_z
