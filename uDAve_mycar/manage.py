...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if camera_type == "stereo":
        ...
        from donkeycar.parts.realsense435ie import RealSense435i_extended
        cam = RealSense435i_extended(
            ...
            driving_mirror_w_1 = cfg.DRIVING_MIRROR_W_1,
            driving_mirror_h_1 = cfg.DRIVING_MIRROR_H_1,
            driving_mirror_w_2 = cfg.DRIVING_MIRROR_W_2,
            driving_mirror_h_2 = cfg.DRIVING_MIRROR_H_2,
        )
        V.add(cam, inputs=[],
              outputs=['cam/image_array', 'cam/image_array_gray','cam/driving_mirror_1','cam/driving_mirror_2',
    ...
    if cfg.LIDAR_1:
        lidar = RPLidar(port=cfg.LIDAR_1_PORT,mode=cfg.LIDAR_1_MODE, pwm=cfg.LIDAR_1_PWM) 
        V.add(lidar, inputs=[],outputs =["lidar/dist_array_l1", "lidar/angle_array_l1", "lidar/quality_array_l1"], threaded=True)

    if cfg.LIDAR_2:
        lidar = RPLidar(port=cfg.LIDAR_2_PORT,mode=cfg.LIDAR_2_MODE, pwm=cfg.LIDAR_2_PWM) 
        V.add(lidar, inputs=[],outputs =["lidar/dist_array_l2", "lidar/angle_array_l2", "lidar/quality_array_l2"], threaded=True)
    ...
    if cfg.NEED_SPEED:
        from donkeycar.parts.speedometer import Speedometer
        V.add(Speedometer(pin_1=cfg.SPEED_PIN_1,pin_2=cfg.SPEED_PIN_2,pin_3=cfg.SPEED_PIN_3,pin_4=cfg.SPEED_PIN_4), outputs=['speed_1', 'speed_2', 'speed_3', 'speed_4'], threaded=False)
    ...
    if cfg.CAMERA_TYPE == "D435" and cfg.REALSENSE_D435_DEPTH:
        inputs += ['cam/image_array_gray','cam/driving_mirror_1','cam/driving_mirror_2',]
        types += ['gray16_array',"pic","pic"]
    ...
    if cfg.HAVE_IMU or (cfg.CAMERA_TYPE == "D435" and cfg.REALSENSE_D435_IMU):
        ...
        types +=['float', 'float', 'float',
           'float', 'float', 'float']
    if cfg.LIDAR_1:
        inputs += ['lidar/angle_array_l1','lidar/dist_array_l1','lidar/quality_array_l1']
        types += ['vector','vector','vector']
    if cfg.LIDAR_2:
        inputs += ['lidar/angle_array_l2','lidar/dist_array_l2','lidar/quality_array_l2']
        types += ['vector','vector','vector']
    if cfg.HAVE_MARVELMIND:
    ...
    if cfg.NEED_SPEED:
        inputs += ['speed_1','speed_2','speed_3','speed_4']
